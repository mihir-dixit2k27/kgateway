...
func TestApplyForRoute_SetsStatPrefix_Static(t *testing.T) {
	...
	// stat_prefix is a field on Route, not on RouteAction
	assert.Equal(t, "my_static_prefix", out.StatPrefix)
}
...
func TestApplyForRoute_SetsStatPrefix_NamespaceAndName(t *testing.T) {
	...
	assert.Equal(t, "production_checkout-route", out.StatPrefix)
}
...
func TestApplyForRoute_SetsStatPrefix_AllVariables(t *testing.T) {
	...
	assert.Equal(t, "staging_api-route_v1-rule", out.StatPrefix)
}
...
func TestApplyForRoute_NilStatPrefix_RouteUnchanged(t *testing.T) {
	...
	assert.Empty(t, out.StatPrefix, "stat_prefix should be empty when not configured")
}
...
func TestApplyForRoute_NilParent_TemplateVariablesUnsubstituted(t *testing.T) {
	...
	assert.Equal(t, "static_value", out.StatPrefix)
}
...
func TestApplyForRoute_RuleNameSubstitution_OnlyWhenSet(t *testing.T) {
	...
	t.Run("RULE_NAME substituted when rule name is set", func(t *testing.T) {
		...
		pCtx := &ir.RouteContext{
			...
			In: ir.HttpRouteRuleMatchIR{
				Parent:   &ir.HttpRouteIR{},
				RuleName: "rule-one", // RuleName is the HTTPRoute rule name used for %RULE_NAME%
			},
		}
		out := makeRouteWithAction()
		require.NoError(t, plugin.ApplyForRoute(pCtx, out))
		assert.Equal(t, "svc_rule-one", out.StatPrefix)
	})
	...
	t.Run("RULE_NAME not substituted when rule name is empty — warning is logged", func(t *testing.T) {
		...
		pCtx := &ir.RouteContext{
			...
			In: ir.HttpRouteRuleMatchIR{
				Parent:   &ir.HttpRouteIR{},
				RuleName: "", // no name — token stays unresolved, warning is logged
			},
		}
		out := makeRouteWithAction()
		require.NoError(t, plugin.ApplyForRoute(pCtx, out))

		// %RULE_NAME% is NOT replaced because RuleName is empty.
		// The literal token stays and a warning is logged by the controller.
		assert.Equal(t, "svc_%RULE_NAME%", out.StatPrefix,
			"unresolved token should remain when rule has no name (operator must check logs)")
	})
}
